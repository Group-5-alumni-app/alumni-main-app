...
Class Action {
	...
	function signup(){
		...
		// If user already exists, stop excecuting.
		if($chk > 0){
			return 2;
			exit;
		}

		// Save new user data to user's table, if the user doest not yet exists.
		$save = $this->db->query("INSERT INTO users set ".$data);
		...
		if($save){
			...
			// Prepare to store data into alumnius_bio table
			$alumnus_bio_data = " firstname = '".$firstname."' ";
			$alumnus_bio_data .= ", middlename = '".$middlename."'";
			$alumnus_bio_data .= ", lastname = '".$lastname."'";
			$alumnus_bio_data .= ", gender = '".$gender."'";
			$alumnus_bio_data .= ", batch = '".$batch."'";
			$alumnus_bio_data .= ", course_id = '".$course_id."' ";
			$alumnus_bio_data .= ", email = '".$email."' ";
			$alumnus_bio_data .= ", connected_to = '$connected_to' ";
			$alumnus_bio_data .= ", avatar = '$fname' ";


			$save_alumni = $this->db->query("INSERT INTO alumnus_bio set ".$alumnus_bio_data );
			if($save_alumni){
